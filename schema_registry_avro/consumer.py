...
schema = schema_registry_client.get_latest_version('btc_price-topic')
...
table_ref = bigquery.TableReference(dataset, 'btc_price')
...
while True:
    msg = consumer.poll(0)
    if msg is not None:
        data = avro_deserializer(msg.value(), SerializationContext('btc_price', 'topic'))
        print(data)
